...
# 辅助函数：翻译扬声器字段
def translate_speakers(text):
    # 检查是否完全匹配
    if text in translations["speakers"]:
        return translations["speakers"][text]
        
    # 部分匹配
    for key, value in translations["speakers"].items():
        if key in text:
            return value
...
for iphone in data:
    ...
    # 3. 更新扬声器 (speakers)
    if isinstance(iphone.get('speakers'), str) and iphone.get('speakers'):
        speakers_zh = iphone['speakers']
        speakers_en = translate_speakers(speakers_zh)
        
        iphone['speakers'] = {
            'zh-CN': speakers_zh,
            'en-US': speakers_en
        }
        fields_updated["speakers"] += 1
        phone_updated = True
