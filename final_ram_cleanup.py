...
def clean_ram_capacity():
    """最终清理iPad JSON文件中的内存容量信息，只保留容量数值，去除LPDDR相关信息，并修复特定型号"""
    ...
    # 特定型号的内存容量修复
    special_fixes = {
        'ipad-ipad': ['256MB'],  # 第一代iPad
        'ipad-ipad-2': ['512MB']  # iPad 2
    }
    
    for ipad in ipad_data:
        # 检查是否是需要特殊处理的型号
        if ipad.get('id') in special_fixes:
            ipad['ram'] = special_fixes[ipad['id']]
            cleaned_count += 1
            print(f"已特殊修复 {ipad['name']} 的内存容量: {ipad['ram']}")
            continue
        ...
        if 'ram' in ipad and isinstance(ipad['ram'], list) and ipad['ram']:
            ...
            for ram_value in ipad['ram']:
                ...
                if 'LPDDR' in ram_value:
                    ...
                    # 如果不包含LPDDR，检查是否包含DDR
                    ddr_match = re.search(r'(\d+(?:\.\d+)?)\s*(MB|GB|TB)\s*DDR', ram_value, re.IGNORECASE)
                    if ddr_match:
                        capacity = ddr_match.group(1) + ddr_match.group(2)
                        cleaned_values.append(capacity)
                        print(f"已清理: {ram_value} -> {capacity}")
                    else:
                        # 如果不包含DDR，保留原值
